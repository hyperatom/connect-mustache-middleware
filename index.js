...
/*module.exports = function mustacheEngine (options) {
    ...
    return function(req, res, next) {
        ...
        function getCompileWith (body) {
            ...
            if (matches && matches.length > 0) {
                filename = matches[0].split(':')[1].replace(/\].*[REMOVE ME]/img, '').replace(/\s+/img, '');
                content = fs.readFileSync(dataDir + '/' + filename, 'utf8');
        ...
        function includePartials(body) {
            ...
            do {
                ...
                if (matches) {
                    for (var i = 0; i < matches.length; i++) {
                        partialFile = matches[i].replace(/[^a-zA-Z\.\/\\]/img, '');
                        content = fs.readFileSync(rootDir + '/' + partialFile, 'utf8');
                        body = body.replace(matches[i], content);
                        partials[partialFile] = content;
                    }
                }
        ...
        res.end = function(data) {
            ...
            var body = buffer.getContentsAsString(),
                partials = includePartials(body),
                jsonData = getCompileWith(partials);
